...
const useMenuListScrollContainer = () => {
  ...
  const [scrollContainer, setScrollContainer] = React.useState<null | React.ComponentRef<typeof MenuList>>(null);
  const ref = React.useCallback<React.RefCallback<React.ComponentRef<typeof MenuListVirtual>>>(el => {
    if (!el) { return; }
    const scrollContainer = el.closest('[data-bk-menu-list-scroller]');
    if (scrollContainer) {
      setScrollContainer(scrollContainer as React.ComponentRef<typeof MenuList>);
    }
  }, []);
...
export type MenuListVirtualProps = Omit<ComponentProps<typeof MenuList.Segment>, 'children'> & {
...
export const MenuListVirtual = (props: MenuListVirtualProps) => {
  const { items, onNearEnd, ...propsRest } = props;
  
  const itemsNormalized: Array<VirtualItemsChunk> = Array.isArray(items) ? items : [items];
  
  const { ref: scrollContainerRef, scrollContainer } = useMenuListScrollContainer();
      ...
      {...mergeProps(
        {
          ...
          className: cl['bk-menu-list-virtual'],
          style: {
        ...
        virtualItems.map(virtualItem =>
          renderItem(
            virtualItem.indexAbsolute,
            {
              ...
              className: cl['bk-menu-list-virtual__item'],
