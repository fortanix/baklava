...
export const entries = <O extends object>(obj: O): Array<EntryOf<O>> =>
  // biome-ignore lint/suspicious/noExplicitAny: purposefully narrowed type
...
// XXX This should only be used if `K` is a literal type, if `K` is something like `string` this breaks the type
...
export const getSingleKey = <O extends object>(obj: O): keyof O => {
  ...
  const singleKey: undefined | keyof O = objKeys[0];
  
  if (objKeys.length !== 1 || typeof singleKey === 'undefined') {
...
export const keyBy = <T, K extends PropertyKey>(items: ReadonlyArray<T>, deriveKey: (item: T) => K): Dict<K, T> => {
  return items.reduce(
    (itemsKeyed, item) => {
