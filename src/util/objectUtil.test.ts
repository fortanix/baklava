...
import * as ObjectUtil from './objectUtil.ts';
...
describe('ObjectUtil', () => {
  test('isObject should resolve true given an object', () => {
    ...
    expect(ObjectUtil.isObject(Number.NaN)).toBe(false);
  ...
  test('isObjectDict should resolve true given an object', () => {
    ...
    expect(ObjectUtil.isObjectDict(Number.NaN)).toBe(false);
  ...
  test('isPlainObject should resolve true given a plain object (prototype: null or Object.prototype)', () => {
    ...
    expect(ObjectUtil.isPlainObject(Number.NaN)).toBe(false);
  ...
  describe('map', () => {
    test('should map the given function over the object properties', () => {
      ...
      expect(objMapped).toStrictEqual({
        a: 0,
        b: 5,
        c: 3,
      });
  ...
  describe('filterWithTypeGuard', () => {
    test('should filter just the properties that satisfy the given predicate function', () => {
      ...
      const objFiltered: ObjectUtil.Dict<string, number> =
        ObjectUtil.filterWithTypeGuard(obj, (x: number): x is 0 | 42 => x >= 0);
    ...
    test('should be immutable', () => {
      ...
      const objFiltered: ObjectUtil.Dict<string, number> = ObjectUtil.filterWithTypeGuard(
        obj,
        (x): x is 0 | 42 => x >= 0,
      );
    ...
    test('should pass key as optional second argument', () => {
      ...
      const objFiltered: ObjectUtil.Dict<keyof typeof obj, ObjectUtil.ValueOf<typeof obj>> =
        ObjectUtil.filterWithTypeGuard(obj, (x, key): x is 'hello' | 'foo' => key !== 'a');
  ...
  describe.skip('sort', () => {}); // TODO
  describe.skip('getSingleKey', () => {}); // TODO
  
  describe('keyBy', () => {
    test('should create an object from an array, keyed by the given deriveKey callback', () => {
      const persons = [
        { id: 'a', name: 'Alice' },
        { id: 'b', name: 'Bob' },
        { id: 'c', name: 'Charlie' },
      ...
      const personsById = ObjectUtil.keyBy(persons, person => person.id);
      
      expect(personsById).toStrictEqual({
        a: { id: 'a', name: 'Alice' },
        b: { id: 'b', name: 'Bob' },
        c: { id: 'c', name: 'Charlie' },
      });
