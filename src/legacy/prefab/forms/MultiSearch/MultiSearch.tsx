...
export const Suggestions = (props: SuggestionProps) => {
  ...
  // do not use FocusTrap because focus doesn't work properly with MultiSearch
  React.useEffect(() => {
    if (!popperElement) return;
    
    if (isDropdownActive) {
      const firstItem = initialFocusSelector
        ? popperElement.querySelector(initialFocusSelector)
        : popperElement.querySelector('.bkl-dropdown__menu-item');
    ...
    // NOTE: it causes a bug to open dropdown again when outside is focused
    //  else {
    //   referenceElement?.focus();
    // }
  }, [isDropdownActive, popperElement, initialFocusSelector]);
  ...
  const renderDropdownItems = () => {
    ...
    return React.Children.map(dropdownChildren, (child, index) => {
      const { onActivate: childOnActivate, value, onClose: childOnClose } = child?.props || {};
      ...
      return child?.type !== SuggestionItem
        ...
        : React.cloneElement(child, {
          ...
          onKeyDown: (evt) => handleOptionKeyDown({
            evt,
            index,
            options: popperElement?.querySelectorAll('.bkl-dropdown__menu-item'),
            triggerElement: referenceElement,
            onClose: handleClose,
            onSelect: () => childOnActivate(value),
          }),
        });
    });
  };
  ...
  const renderDropdown = () => {
    return (
      ...
      >
        <ul className="bkl-dropdown__menu" role="menu">
          {renderDropdownItems()}
        </ul>
...
export const SearchInput = React.forwardRef<HTMLInputElement, MultiSearchInputProps>((props, ref) => {
  ...
  const isInputActive = isFocused || isInputFocused;
  ...
  return (
    <div className={cx('bkl-search-input', className, { 'bkl-search-input--active': isInputActive })}>
...
const DateTimeDropdown = (props: DateTimeDropdownProps) => {
  ...
  return (
    <Suggestions
      ...
      onOutsideClick={onOutsideClick}
      // avoid focus when it's not opened by keyboard not to display DatePicker
      initialFocusSelector={isOpenedByKeyboard ? '.bkl-input__input' : ''}
      basic
    >
...
type OperatorsDropdownProps = {
  ...
  onOutsideClick?: () => void,
  setIsOpenedByKeyboard?: (isOpenedByKeyboard: boolean) => void,
  operatorInfo?: OperatorInfo,
};
...
const OperatorsDropdown = (props: OperatorsDropdownProps) => {
  ...
  // check whether operator was clicked by mouse or keyboard to focus DatePicker only for keyboard accessibility
  // (avoid showing DatePicker unnecessary when it's controlled by mouse)
  React.useEffect(() => {
    if (!setIsOpenedByKeyboard) return;
    const handleKeyDown = () => {
