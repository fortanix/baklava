...
Note: for linking to other pages, there's `addon-links`, but it doesn't allow linking to a docs page. Instead:
  - Use `../?path=` to escape the `iframe.html`
  - Use `#storybook-docs` to fix the scroll restoration issue
https://github.com/storybookjs/storybook/issues/8618
*/}

- [Prose](../?path=/docs/typography-prose--docs#storybook-docs)
